...
class FormationController extends AbstractController
{
     ...
     */
    public function list (MyformationRepository $formationRepository)
    {
        $em = $this->getDoctrine()->getManager();
        ...
        $a=$formationRepository->filter("livrable");
        $i=0;
        foreach ($a as $row){
            $i++;
        }
        $b=$formationRepository->filter("video");
        $j=0;
        foreach ($b as $row){
            $j++;
        }
    ...
    public function deleteFormation($id){
        ...
        foreach ($files as $file){
           ...
           //$this->getParameter('brochures_directory').'/'.$nom);
            ...
            $em->remove($file);
            $em->flush();
            $this->addFlash('info', 'formation supprimé avec succés');
